...
public class MySqlDialect implements Dialect {
    ...
    public String getCountSql(String sql) {
        ...
        if (select.toLowerCase().indexOf("select distinct") != -1
            ...
            return new StringBuffer(countSelect.length()).append("select count(1) count from (")
                    .append(countSelect.substring(0, orderIndex)).append(" ) t").toString();
        } else {
            return new StringBuffer(countSelect.length()).append("select count(1) count ")
                    .append(countSelect.substring(formIndex, orderIndex)).toString();
        }
    }
    ...
    private static int getLastOrderInsertPoint(String querySelect) {
        ...
        if (orderIndex == -1 || !isBracketCanPartnership(
                querySelect.substring(orderIndex, querySelect.length()))) {
