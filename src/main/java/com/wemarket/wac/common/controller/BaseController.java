package com.wemarket.wac.common.controller;
...
public abstract class BaseController {
    ...
    protected <E extends BaseDTO, T> DeferredResult<T> execute(
        ...
        try {
            ...
            if (m != null && (m instanceof BaseResponseDTO)) {
                ((BaseResponseDTO) m)
                        .setResponseStatus(BaseResponseStatus.FRONT_INTERNAL_SERVER_ERROR);
            }
    ...
    protected <E extends BaseDTO> BaseResponseDTO preService(E requestDto,
        ...
        message.setResponseStatus(BaseResponseStatus.SUCCESS);
        return message;
    }
    ...
    private class DeferredResultRunnable<E extends BaseDTO, T> implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                // 1.2 安全检测不过
                if (!BaseResponseStatus.SUCCESS.getCode().equals(preServiceResult.getErrcode())) {
                    T returnMsg = exceptionReturnDefaultObject;
                    if (!BaseResponseStatus.REQUEST_UNSAFE.getCode()
    ...
    protected <T> T handleFailResponseMessage(Exception e, T exceptionReturndefaultWebMessage) {
        ...
        if (m != null && (m instanceof BaseResponseDTO)) {
            ((BaseResponseDTO) m).setResponseStatus(BaseResponseStatus.FRONT_INTERNAL_SERVER_ERROR);
        }
        return m;
    }
